...
namespace NhatDaiShop.Web.Controllers
{
    public class ProductController : Controller
    {
        ...
        public ActionResult Category(int id, int page = 1, string sort = "")
        {
            ...
            int totalRow = 0;
            var productModel = _productService.GetListProducByCategoryIdPaging(id, page, pageSize, sort, out totalRow);
            IMapper mapper = AutoMapperConfiguragtion.Mapper;
            var productViewModel = mapper.Map<IEnumerable<Product>, IEnumerable<ProductViewModel>>(productModel);
            int totalPage = (int)Math.Ceiling((double)totalRow / pageSize);

            var catogory = _productCategoryService.GetById(id);
            ViewBag.Category = mapper.Map<ProductCategory, ProductCategoryViewModel>(catogory);
